...
1. Js files were translated into browser-readable versions(via `gulp-babel`) with all dependencies(via `gulp-browserify`) and combined into a single js file(via gulp-concat).
1. Both CSS and JS are all minified to improve loading speed, but `gulp-sourcemaps` still allows them to be reverted back during console debugging.
1. Reduced amount of configurations needed (previously I was using babel, uglify and browserify separately).
...
1. SCSS was learned with practice, a style is only stylish when it's reusable.
1. Experience in using Gulp was gained, which will save me some time in future building.
1. More node.js practices were done while developing the backend. Also bugs in existing code were discovered and fixed along the way. Improvements were also implemented due to richer node.js experience.
