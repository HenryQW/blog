...
`Schedule` 代表这个 agent 自动运行的频率，将第一个 `Website Agent` 设置为每天早上 7 点自动运行（或更高频率如每 5 小时）。 第二个 `Website Agent` 设置为 `never` ，但勾选 `Propagate immediately` ，代表**每当新事件输入即自动运行**。
